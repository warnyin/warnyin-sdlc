---
id: final-gate-confirm-skip
tier: standard
status: shipped
---
# Change: The final gate asks before running the full suite, and vibe changes skip it
<!-- cap:100 · standard tier. Delta sections ARE the spec change — merged mechanically at ship, never re-narrated. -->

## Why (≤5 lines)
The final gate always runs the full test command. For a slow suite that is a long wait even when
the human knows CI will run it anyway, and a vibe change (≤2 files, reversible) pays the same
price as a deep one. The human asked on 2026-09-18 for vibe to skip the final gate, and for the
full run to be confirmed first, with the option to skip it.

## Assumptions
- Tier `standard`: playbook doctrine, the auto escalation table, one observe counter. No hook,
  installer, prune or merge semantics (`harness.md § Tier triage`).
- Vibe skips without asking; the human chose it (option 2 on 2026-09-18). Safe: vibe is ≤2 files
  and reversible by the triage table.
- Standard and deep ask run or skip, run recommended. Deep and hard-floor are skippable too, as
  the human asked; the digest names every skip, so the reader weighs it.
- No question when the fast gate's full run is reused: nothing would run. Safe: same command ran.
- Unattended runs gather the choice in the single up-front confirmation (a new escalation row),
  never mid-run. Safe: `auto.md` already forbids mid-run questions outside that set.
- A skipped final gate still sets `verified`, recorded as `result=skipped`; it is neither a
  round nor a failure. Safe: the digest and journal keep it distinguishable from a pass.
- A build after a skipped final gate sends ship back to verify, as after a pass.
- No lens: pipeline doctrine only.

## Delta: verify-gates

### MODIFIED Requirement: The full suite runs once, after review
...
### ADDED Requirement: The full run is confirmed, and a vibe change skips it
...
- [x] T1 verify.md final gate: vibe skips, others ask run/skip (run recommended), reuse asks nothing, `result=skipped by=<tier|human>` note [tier:balanced]
- [x] T2 auto.md escalation row; ship.md: build after a final pass or skip → verify, digest names a skip; rules-card Verify line [P] [tier:cheap]
- [x] T3 lib/observe.mjs: a skipped final gate is not a round and leaves first-pass alone [P] [tier:cheap]
- [x] T4 CHANGELOG Unreleased: final gate confirmation and vibe skip [P] [tier:cheap]
