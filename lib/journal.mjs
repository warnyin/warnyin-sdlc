...
export function appendEvent(filePath, event) {
  ...
  // Containment is the caller's check (`appendJournal`); this is the second layer. `O_NOFOLLOW`
  // refuses only a symlink at the FINAL component — it does not stop traversal through a
  // symlinked directory — so it narrows the window between that check and this write rather
  // than replacing it. Same two layers the change-file write has, not a weaker one for telemetry.
  ...
  let fd;
  try {
    fd = fs.openSync(filePath, fs.constants.O_WRONLY | fs.constants.O_APPEND | fs.constants.O_CREAT | noFollow);
...
// ---------- reading what an autopilot run recorded ----------
// Event fields arrive as free `k=v` text. Every reader that decides whether a human's "no" was
// honoured reads them through these, fail-closed: casing never changes what an event is, and a
// hard-floor flag that is not exactly `no` or `approved` counts as an unapproved hard floor.

export function foldValue(value) {
  return String(value ?? '').trim().toLowerCase();
}

export function isPilotEscalation(e) {
  return !!e && typeof e === 'object' && foldValue(e.event) === 'escalation' && foldValue(e.preauth) === 'pilot';
}

export function isDelegation(e) {
  return !!e && typeof e === 'object' && foldValue(e.event) === 'delegation';
}

// 'no' · 'approved' (a surface the human approved up front in grill.md § Mandate) · 'yes'.
export function hardFloorLevel(e) {
  const v = foldValue(e?.hardfloor);
  return v === 'no' || v === 'approved' ? v : 'yes';
}
