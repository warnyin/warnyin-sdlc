...
import { readChangeJournal, readJournalFile, sealedJournalPath, globalJournalPath, foldValue, isPilotEscalation } from './journal.mjs';
...
function summarizeChange(sdlcRoot, dir, id, archived) {
  ...
  let preauthorized = 0;
  // escalations the agent decided alone under /sdlc:autopilot, with no human pre-approval.
  let pilot = 0;
  let guards = 0;
  let compacts = 0;

  // A session record is that session's running total, written at every Stop: only the
  // latest per session counts. A record with no session id stands on its own.
  const latest = new Map();
  events.forEach((e, i) => {
    if (e.event === 'session') latest.set(e.session ? `s:${e.session}` : `i:${i}`, e);
  });
  ...
  for (const e of events) {
    if (foldValue(e.event) === 'escalation' && foldValue(e.preauth) === 'yes') preauthorized++;
    if (isPilotEscalation(e)) pilot++;
  ...
  return {
    ...
    verify, selfJudged, preauthorized, pilot, guards, compacts, leadTimeMs, shippedAt,
