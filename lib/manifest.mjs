...
// `.kimi-code/skills/` is a directory the USER also keeps their own skills in — it is Kimi's
// own convention, unlike `.cursor/rules/sdlc.mdc` which only we ever write. So the allowlist is
// our name prefix and our filename, never the tree: a user's skill is out of prune's scope
// entirely, rather than relying on the disk-hash guard to spare it.
const KIMI_SKILL_ALLOW_RE = /^\.kimi-code\/skills\/sdlc-[^/]+\/SKILL\.md$/;
...
export function isPrunablePath(relPosix) {
  ...
  if (KIMI_SKILL_ALLOW_RE.test(relPosix)) return true;
  return false;
}
