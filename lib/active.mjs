...
import { listOpenChangeIds, parkReason } from './relations.mjs';
import { parseFrontmatter } from './frontmatter.mjs';
import { isRealPathInside, hasEntry } from './safe-path.mjs';
...
function mostRecentOpenChange(sdlcRoot) {
  ...
  for (const id of listOpenChangeIds(sdlcRoot)) {
    ...
    try {
      ...
      // Parking rewrites change.md, so without this the freshly parked change would win the
      // very fallback that exists to stop offering it.
      if (parkReason(parseFrontmatter(fs.readFileSync(p, 'utf8')).data.parked)) continue;
    } catch {
      continue;
    }
